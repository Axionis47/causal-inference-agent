...
export default function QuestionList({
  ...
  return (
    <div className="qs" aria-label="Open questions">
      {questions.map((q, i) => {
        const chips = q.kind === "choose" ? q.options : q.kind === "confirm" ? (q.options.length ? q.options : ["Yes, that's right", "No"]) : [];
        return (
          <div className="qrow" key={i}>
            <span className="n">{i + 1}.</span>
            <span className="t">
              {q.text}
              {q.because.length > 0 && <span className="why"> · asked for {q.because.join(", ")}</span>}
            </span>
            {chips.map((c) => {
