...
describe("warnings", () => {
  it("collects the dataset's facts as short lines", () => {
    expect(datasetWarnings(prof({ duplicate_rows: 1, co_missing: [["a", "b"]], issues: ["one or more headers have leading or trailing whitespace"] }))).toEqual(
      ["one or more headers have leading or trailing whitespace", "1 duplicate row", "a and b are missing on the same rows"],
    );
  ...
  it("collects a column's oddities", () => {
    expect(
      columnWarnings(
        col({
          constant: true,
          sentinels: [{ value: "-999", count: 4, reason: "placeholder number at the edge of the range" }],
          issues: ["values have leading or trailing whitespace"],
        }),
      ),
    ).toEqual(["values have leading or trailing whitespace", "every row has the same value", "-999 × 4: placeholder number at the edge of the range"]);
  });
});
