...
from causal_agent.viz.spec import Edge, FigureSpec, Mark, Node, Series

ROLE_WORDS = {"confounder": "drives both", "driver": "drives the outcome", "treatment driver": "drives the treatment", "mediator": "carries the effect",
              "instrument": "pushes units in", "hidden": "not in the file", "excluded": "set aside"}


def _role(graph: dict, node: str) -> str:
...
def causal_graph(graph: dict, names: dict[str, str] | None = None) -> FigureSpec | None:
    ...
    nodes += [Node(id=x["column"], label=names.get(x["column"], x["column"]), role="excluded") for x in graph.get("excluded") or [] if x.get("column") not in graph["nodes"]]
    ...
    note = ("adjusted for " + ", ".join(conf) if conf else "nothing drives both the treatment and the outcome in this graph") + ("; a hidden factor is drawn in" if any(n.role == "hidden" for n in nodes) else "")
    return FigureSpec(id="causal_graph", kind="graph", title="The graph the analysis drew", nodes=nodes, edges=edges, note=note, draws_on=["design.graph"] + cites)
...
def balance(facts: dict[str, dict], contrast: str, threshold: float | None = None, names: dict[str, str] | None = None) -> FigureSpec | None:
    ...
    return FigureSpec(id=f"balance_{contrast}", kind="bars", title="How alike the arms are on what was adjusted for", x_label="", y_label="standardised mean difference",
                      series=[Series(name="before adjustment", x=x, y=before), Series(name="after weighting on the score", x=x, y=after)], marks=marks,
                      note=f"after weighting the largest difference is {worst:.2f}", draws_on=[f"check:{contrast}.balance.{c}" for c in cols] + ["check_facts.balance"])
