...
def overlap_probe(df: pd.DataFrame, treated: pd.Series, columns: list[str], floor: int) -> Probe:
    ...
    detail = (f"{len(t)} cells over {', '.join(columns) or 'no columns'}; smallest arm in a cell {smallest}; "
...
def overlap(df: pd.DataFrame, treatment: str, treated_level: str, columns: list[str], floor: int = 20, addresses: list[str] | None = None) -> Figure:
    ...
    spec = FigureSpec(
        id=f"overlap_{'_'.join(_key(c) for c in columns)}", kind="bars", title=f"Who got the change, by {', '.join(columns)}",
        x_label="level", y_label="share of the arm",
        series=[Series(name=f"{treatment} = {treated_level}", x=list(x), y=ys["treated"], n=ns["treated"]),
                Series(name=f"{treatment} ≠ {treated_level}", x=list(x), y=ys["control"], n=ns["control"])],
        note=("both arms appear at every level" if probe.passed else "some level has one arm thin or missing") + f"; smallest cell {int(probe.value)} rows",
