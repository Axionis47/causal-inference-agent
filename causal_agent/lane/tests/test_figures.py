...
def test_ok_addresses_and_write(tmp_path):
    ...
    state = {"checks": [CheckResult(contrast="c", name="overlap", level="pass")],
             ...
             "check_facts": {"balance": {}}, "declines": []}
    ok = F.ok_addresses(h, state, "refute")
    assert {"col:lunch.when", "check:c.overlap", "estimate:c.value", "estimate:c.psw.value", "refute:c.placebo_treatment_refuter.new_effect", "check_facts.balance", "design.graph"} <= ok
