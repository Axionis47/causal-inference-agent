...
EMITTED = {
    ...
    "rd": ["sides", "effective_rows", "density", "mass_points", "support", "compliance", "first_stage", "first_stage_weak", "no_first_stage", "covariate_continuity"],
}
...
def test_a_per_column_check_fills_the_column_and_an_unknown_check_keeps_its_name():
    ...
    assert "parental level of education" in W.check_words(cfg, "balance.parental_level_of_education", {"parental_level_of_education": "parental level of education"})
...
def test_say_leads_the_detail_with_the_sentence_and_leaves_the_persons_flags_alone():
    cfg = dowhy_checks()
    rs = [CheckResult(contrast="c", name="balance.lunch", level="soft", detail="standardised mean difference 0.16"),
