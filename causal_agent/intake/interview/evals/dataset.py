...
def main() -> None:
    ...
    if client.has_dataset(dataset_name=DATASET):
        ...
        ds = client.create_dataset(dataset_name=DATASET, description="Intake interview: scripted conversations that must settle the claims and raise the ready flag")
    ...
    if new:
        client.create_examples(
            dataset_id=ds.id,
            inputs=[{"csv": c["csv"], "context": c["context_text"], "turns": c["turns"], "name": c["id"]} for c in new],
