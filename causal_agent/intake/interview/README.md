...
`intake/knowledge/claims.yaml` declares ten kinds: `grain`, `sampling`, `change`, `assignment`, `measured` (one per column), `missing`, and four the file can never check, `unobserved`, `exclusion`, `spillover`, `trend_continues`. Each kind has typed fields, the check that can refute it, and a question frame in world terms. `family_needs` says which kinds each family requires and which values fit; that block is read by code to build the table. No family or method word appears in a frame.
