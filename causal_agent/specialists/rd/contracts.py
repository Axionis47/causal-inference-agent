...
class Score(BaseModel):
    ...
    cutoff_value_treated: bool = Field(default=True, description="true if a unit whose score equals the cutoff exactly got the change (an 'at or above' or 'at or below' rule); false if the rule is strict")
    takeup_column: str | None = Field(default=None, description="the column recording whether the unit actually received the change, or null when the data records none and treatment is the cutoff rule itself")
    takeup_level: str | None = Field(default=None, description="the exact value of the take-up column meaning the unit received the change; null when takeup_column is null")
    reason: str
    cites: list[str]
...
class ShapeFacts(BaseModel):
    ...
    rows_at_cutoff: int
    cutoff_shift: float = Field(description="how far the effective cutoff moved, in the score's units, so that units exactly at the cutoff fall on the side the notes declare; 0 when none sit there")
    score_min: float
    score_max: float
...
class CovariateRelation(BaseModel):
    ...
    column: str
    predetermined: bool = Field(description="the column's value was fixed before the score was set and the change decided, so it must be continuous at the cutoff")
    affected_by_treatment: bool = Field(description="the column's value could have been changed by the treatment; adjusting for it would remove part of the effect")
...
class DesignAssessment(BaseModel):
    ...
    reason: str = Field(description="one or two sentences citing the flag numbers and, for density or covariate flags, the note that argues for or against them")
...
class Bandwidths(BaseModel):
    ...
    rule: Literal["mse", "support_points"] = Field(default="mse", description="mse: the library's MSE-optimal choice; support_points: the score has few distinct values, so h is the distance that keeps a declared number of support points on each side")
    n_h_left: int = 0
    n_h_right: int = 0
...
class Design(BaseModel):
    ...
    def render(self) -> str:
        ...
        lines = [
            "DESIGN",
            f"  score        {s.column} treated when {rule}" + (f"; take-up {s.takeup_column} = {s.takeup_level!r}" if s.takeup_column else "; treatment is the cutoff rule itself") + f" ({s.reason})",
        ...
        lines += ["  " + l for l in self.covariates.render().splitlines()]
        ...
        bw = self.bandwidths
        lines.append(f"  bandwidths   {bw.rule}: h {bw.h:.4g}  b {bw.b:.4g}" + (f"  h_cer {bw.h_cer:.4g}" if bw.h_cer is not None else "") + f"  effective rows {bw.n_h_left}/{bw.n_h_right}"
...
class RDInterpretation(Interpretation):
    ...
    estimand: Estimand = Field(description="which quantity the estimate is: effect_at_cutoff (sharp), complier_effect_at_cutoff (fuzzy), or itt_at_cutoff (effect of crossing the cutoff, whatever was taken up)")
