...
def run_checks(panel: pd.DataFrame, shape: ShapeFacts, controls: list[str], contrast_key: str, cfg: dict[str, Any]) -> list[CheckResult]:
    ...
    if shape.periods_pre <= cfg["periods"]["parallel_untestable_when_pre"]:
        ...
        out.append(_pre_trends(panel, controls, contrast_key, cfg))
    if shape.cohorts > 1:
...
def _pre_trends(panel: pd.DataFrame, controls: list[str], contrast_key: str, cfg: dict[str, Any]) -> CheckResult:
    ...
    if res is None:
        return CheckResult(contrast=contrast_key, name="pre_trends", level="soft", detail="no pre-period coefficients to test")
    stat, p, k = res
