...
from tests.estimation.test_engine import PACKS, REGISTRIES, ROLES, make_plan
...
def test_a_design_conflict_is_committed_and_returned_to_prd_002(
    ...
    assert out["status"] == "design_conflict" and out["conflict_code"] == CAPACITY_EXCEEDED
    ...
    assert body["conflict_code"] == CAPACITY_EXCEEDED
    ...
    assert base.close_run(out).conflict_code == CAPACITY_EXCEEDED
