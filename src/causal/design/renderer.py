...
from causal.design.frame import CausalGraphViewV1, GraphEdgeViewV1, GraphNodeViewV1
from causal.design.semantics import CausalContextV1, ConceptStatus, MeasurementMapV1, RoleLedgerV1
from causal.shared.canonical import content_hash
from causal.shared.contracts import ArtifactRef
from causal.shared.envelope import EpistemicStatus

__all__ = ["GRAPH_VIEW_INFIDELITY", "LEGEND_TEXT", "RENDERER_UNAVAILABLE", "GraphSpec",
           "RendererError", "build_graph_spec", "graphviz_version", "render_causal_graph"]
...
def graphviz_version() -> str | None:
    """The actual local Graphviz version from `dot -V`; None when the binary is absent (D-042)."""
...
def build_graph_spec(context: CausalContextV1, ledger: RoleLedgerV1, measurements: MeasurementMapV1,
    ...
    for node in nodes:
        digraph.node(node.concept_id, label=node.label,
    ...
    for view in edges:
        ...
        digraph.edge(view.source_concept_id, view.target_concept_id, style=line,
...
def _check_fidelity(svg: str, dot: str, table: str, spec: GraphSpec) -> None:
    nodes, edges, _, _ = spec
    plain = dot.replace('"', "")
    lost = [n.concept_id for n in nodes
            if n.concept_id not in svg or f"{n.concept_id} | {n.status.value} |" not in table]
    pairs = {e.edge_id: f"{e.source_concept_id} -> {e.target_concept_id}" for e in edges}
    lost += [e.edge_id for e in edges if pairs[e.edge_id] not in plain
             or f"{pairs[e.edge_id]} | {e.status.value}" not in table]
    if lost:
