...
class ColumnMeasurementV1(_Row):
    label: str
    units: str | None
    scale: str | None
...
class _ApprovedSelection(_Payload):
    ...
    column_measurements: dict[str, ColumnMeasurementV1] = {}
    row_set_hash: Sha256Hex
    contrast_ids: _Ids
    required_sensitivity_ids: tuple[Identity, ...]
    figure_builder_ids: tuple[Identity, ...] = ()  # historical reads only; new public plans are numerical
...
class EstimationContextManifestV1(_ApprovedSelection):
    ...
    method_structure: ValueMap = {}
    preprocessing_rule_ids: tuple[Identity, ...]
    result_cardinalities: CountMap
    required_visual_evidence_ids: tuple[Identity, ...]
...
class AnalysisSupportingDataV1(_Lineage):
    """Returned scientific measurements, independent of any reporting decisions."""

    schema_version: Literal["analysis-supporting-data.v1"] = "analysis-supporting-data.v1"
    plan: ArtifactRef
    primary_result: ArtifactRef
    measurements: dict[str, ValueMap]
    unavailable_values: tuple[str, ...] = ()
    contributing_counts: CountMap
    contribution_mask_hash: Sha256Hex | None


class NumericalBundleV1(_Lineage):
    """Numerical-only handoff; historical EstimationBundleV1 remains readable."""

    schema_version: Literal["numerical-bundle.v1"] = "numerical-bundle.v1"
    compiled_design: ArtifactRef
    prepared_bundle: ArtifactRef
    row_set_hash: Sha256Hex
    context_manifest: ArtifactRef
    ...
    primary_result: ArtifactRef
    multiplicity_result: ArtifactRef | None
    # Diagnostic then sensitivity: no chart, judgment, or delivery requirement.
    evidence_bundles: Annotated[tuple[ArtifactRef, ...], Field(min_length=2, max_length=2)]
    supporting_data: ArtifactRef
    numerical_environment: ArtifactRef
