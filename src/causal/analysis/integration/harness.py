...
from causal.analysis.integration.packs import EstimationPackRegistry, EstimationPackV1
...
# Numerical stages, with no model or reporting phase.
PHASES: Final = ("entry", "plan", "estimate", "evidence", "close")
...
EstimationState = TypedDict("EstimationState", {
    ...
    "row_set_hash": str, "overall_ceiling": str,
...
def role_columns(design: Mapping[str, Any], columns: Sequence[str]) -> dict[str, str]:
    """Flatten compiler-owned bindings without dropping additional role columns."""
    known, roles = set(columns), {}
    for binding in design.get("role_bindings") or ():
        role = str(binding["role"])
        found = [str(name) for name in binding.get("columns") or () if str(name) in known]
        for index, column in enumerate(found, start=1):
            roles[role if index == 1 else f"{role}__{index}"] = column
    return roles
...
def input_types(frame: pl.DataFrame, roles: Mapping[str, str]) -> dict[str, str]:
    ...
    return {role: "identifier" if role.partition("__")[0] in IDENTIFIER_ROLES
            else "time" if role.partition("__")[0] in TIME_ROLES
            else "binary" if role.partition("__")[0] in BINARY_ROLES
            and frame[column].n_unique() <= 2
            else "categorical"
            if role.partition("__")[0] in CATEGORICAL_ROLES
            or not frame.schema[column].is_numeric() else "numeric"
...
class HarnessBase:
    ...
    def entry_inputs(self, state: EstimationState, outcome: ArtifactEnvelopeV1
        ...
        held = (body["prepared_bundle"], bundle["compiled_design"],
        ...
        roles = role_columns(design, frame.columns)
        ...
        return plancompile.EntryInputs(
            ...
            capacity=self.payload(entries[2].artifact_id),
