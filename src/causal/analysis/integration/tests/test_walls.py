...
def test_only_numerical_walls_are_loaded_from_the_legacy_registry() -> None:
    assert {rule.wall for rule in RULES} == {1, 2, 4, 5, 6, 7, 8, 9, 10, 11}
    assert {rule.kind for rule in RULES} == set(ew.RULE_KINDS)
    assert ValidationReport(wall=ew.MAX_WALL, issues=()).wall == 11
...
def test_every_wall_passes_on_a_complete_green_context() -> None:
    ...
    assert all(ew.wall(number, context()).passed for number in {row.wall for row in RULES})
    assert ew.wall(4, context(estimator_input_types={
        "treatment": "categorical", "outcome": "numeric",
        "precision_covariate": "numeric", "precision_covariate__2": "numeric"})).passed
...
def test_rct_legacy_cr2_alias_accepts_only_the_exact_truthful_correction_label() -> None:
    ...
    truthful = item().model_copy(update={"finite_sample_correction": "hc2_cr1"})
    assert ew.wall(8, context(plan=legacy, primary_result=result((truthful,)))).passed
    wrong = truthful.model_copy(update={"finite_sample_correction": "hc3_cr3"})
    assert not ew.wall(8, context(plan=legacy, primary_result=result((wrong,)))).passed
    obsolete = truthful.model_copy(update={"finite_sample_correction": "hc2_cr2"})
    assert not ew.wall(8, context(primary_result=result((obsolete,)))).passed


@pytest.mark.parametrize(("method", "role", "name", "status", "policy", "present", "qualified"), [
    ("sharp_rdd", None, "covariate_continuity", "not_computable", "warning", True, True),
    ("sharp_rdd", "predetermined_covariate", "covariate_continuity", "not_computable", "warning", True, False),
    ("sharp_rdd", "predetermined_covariate__2", "covariate_continuity", "not_computable", "warning", True, False),
    ("sharp_rdd", "precision_covariate", "covariate_continuity", "not_computable", "warning", True, False),
    ("sharp_rdd", None, "covariate_continuity", "failed", "warning", True, False),
    ("sharp_rdd", None, "covariate_continuity", "not_computable", "acceptable", True, False),
    ("sharp_rdd", None, "covariate_continuity", "not_computable", "warning", False, False),
    ("sharp_rdd", None, "density_manipulation_test", "not_computable", "warning", True, False),
    ("randomized_experiment", None, "covariate_continuity", "not_computable", "warning", True, False),
])
def test_an_explicit_diagnostic_terminal_status_preserves_evidence(
        method: str, role: str | None, name: str, status: str, policy: str,
        present: bool, qualified: bool) -> None:
    from causal.analysis.tests.support.builders import make_diagnostic

    plan = PLAN.model_copy(update={"method_id": method,
        "role_columns": {role: "baseline"} if role else {},
        "required_diagnostics": {name: "qualification_guard"}})
    rows = (make_diagnostic(name, "qualification_guard", status=status, policy=policy),) if present else ()
    assert ew.wall(9, context(plan=plan, diagnostics=rows)).passed is present
...
def test_an_earlier_failure_stops_every_later_wall() -> None:
    report = ew.validate(ew.MAX_WALL, context(handoff_accepted=False))
