...
from causal.analysis.integration.contracts import (
    DiagnosticSeverity,
    EstimationError,
    ValueMap,
    _Ids,
...
IMPLEMENTED_PACKS: Final = frozenset({
...
# Analysis loads its own four implementations; an optional upstream catalog can be checked too.
def load_estimation_packs(path: Path | None = None,
                          method_packs_path: Path | None = None) -> EstimationPackRegistry:
    path = path if path is not None else RESOURCE_ROOT / "method-pack-estimation.v1.json"
    try:
        ...
        raise EstimationError(f"invalid registry file {path}: {error}",
                              INVALID_PACK_FILE) from error
    known = (_design_pack_keys(method_packs_path) if method_packs_path is not None
             else set(IMPLEMENTED_PACKS))
    return EstimationPackRegistry(parsed.packs, known)
