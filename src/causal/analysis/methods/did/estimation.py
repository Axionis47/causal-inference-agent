...
# harvest read off the same fits and one polars pivot, and the §11.4 prespecified branches.
# `pandas` appears only at the pyfixest boundary. This module
...
from causal.analysis.methods.did import diagnostics
...
def _adoption(view: pl.DataFrame, roles: Mapping[str, str], params: ec.ValueMap) -> pl.Expr:
    ...
    named = str(params.get("adoption_time") or "")
    if named in view.columns:
        return pl.when(pl.col(named) > 0).then(pl.col(named)).otherwise(None).alias(ADOPT)
    with suppress(ValueError):
        threshold = float(named)
        ever = (pl.col(roles["treatment"]).cast(pl.Float64) > 0).max().over(roles["unit_identifier"])
        return pl.when(ever).then(pl.lit(threshold)).otherwise(None).alias(ADOPT)
...
def _timing(view: pl.DataFrame, roles: Mapping[str, str], params: ec.ValueMap) -> pl.DataFrame:
    ...
    adoption = str(params.get("adoption_time") or "")
    numeric = [roles[role] for role in ("outcome", "time") if roles.get(role) in view.columns]
    numeric += [adoption] if adoption in view.columns else []
    frame = view.with_columns(pl.col(name).cast(pl.Float64) for name in numeric)
    frame = frame.with_columns(_adoption(frame, roles, params))
    if placebo:
...
def series(frame: pl.DataFrame, cells: pl.DataFrame, roles: Mapping[str, str],
           plan: ec.EstimationPlanV1, fitted: Feols | None) -> dict[str, ec.ValueMap]:
    found: dict[str, ec.ValueMap] = {}
    time = roles["time"]
...
class DifferenceInDifferencesAdapter:
    ...
    def _fitted(self, frame: pl.DataFrame, roles: Mapping[str, str], plan: ec.EstimationPlanV1,
                params: ec.ValueMap, profile: str, *,
                compute_diagnostics: bool = True) -> engine.AdapterResult:
        ...
        cells = _cells(frame, roles)
        # A simultaneous primary does not depend on an event-study diagnostic fit.
        # The legacy eager path still computes both; standalone collection runs it later.
        fitted = _event_fit(frame, roles) if compute_diagnostics or profile == STAGGERED else None
        aggregation = str(params.get("aggregation") or "cohort_weighted_overall")
        quantities: ec.ValueMap = {
            "adoption_profile_id": profile,
            "fitted_cells": len(_coefficients(fitted)) if fitted is not None else None,
        ...
        return engine.AdapterResult(
            (item,), diagnostics.harvest(frame, cells, roles, plan, params, fitted)
            if compute_diagnostics else {}, fitted)
